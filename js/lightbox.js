...
;(function($){
    var LightBox=function(settings){
        ...
        this.captionText=this.popupWin.find('span.lightbox-pic-desc');//图片描述
        ...
        this.bodyNode.delegate('.js-lightbox,*[data-role=lightbox]','click',function(e){
            //阻止事件冒泡
            e.stopPropagation();
            var currentGroupName =$(this).parent().next().attr('data-group');
    ...
    LightBox.prototype={
        ...
        goto:function(dir){
            if(dir==='next'){
                ...
                if(this.index != 0){//如果索引不为0，则让pre按钮可用
                    this.prevBtn.addClass('lightbox-prev-btn-show').removeClass('disabled');
                }
                //拿下一个要显示的图片的地址
                ...
                if(this.index !=this.groupData.length-1){
                    this.nextBtn.addClass('lightbox-next-btn-show').removeClass('disabled');
                };
                var src=this.groupData[this.index].src;
                this.loadPicSize(src);
            }
        ...
        initPopup:function(currentObj){
            ...
            sourceSrc=currentObj.parent().next().attr('data-source');
            currentId=currentObj.parent().next().attr('data-id');
